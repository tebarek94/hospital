...
export default function deletePatientHistoryController(req, res) {
    const { id } = req.params; // Extracting the patient history ID from request parameters

    // SQL query to delete a patient history record
    ...
    db.query(sql, [id], (err, result) => {
        ...
        return res.status(200).json({ message: "Patient history deleted successfully" });
    });
}